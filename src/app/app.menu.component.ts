...
export class AppMenuComponent implements OnInit {
  ...
  ngOnInit() {
    // ngScaffolding
    this.menuQuery.select(menuState => menuState.menuItems).subscribe(items => (this.menuItems = items));

    this.menuQuery.selectError().subscribe(err => {
      if (err) {
        this.logger.error(err);
        this.messageService.add({
          severity: 'error',
          summary: 'Error',
          detail: 'Unable to get Menu Details'
        });
      }
    });
    // ngScaffolding
            ...
            items: [
              {
                label: 'Submenu 1.1',
                icon: 'subject',
                items: [{ label: 'Submenu 1.1.1', icon: 'subject' }, { label: 'Submenu 1.1.2', icon: 'subject' }, { label: 'Submenu 1.1.3', icon: 'subject' }]
              },
              {
                label: 'Submenu 1.2',
                icon: 'subject',
            ...
            items: [
              {
                label: 'Submenu 2.1',
                icon: 'subject',
                items: [{ label: 'Submenu 2.1.1', icon: 'subject' }, { label: 'Submenu 2.1.2', icon: 'subject' }, { label: 'Submenu 2.1.3', icon: 'subject' }]
              },
              {
                label: 'Submenu 2.2',
                icon: 'subject',
...
export class AppSubMenuComponent {
  ...
  onMouseEnter(index: number) {
    if (this.root && this.app.menuHoverActive && (this.app.isHorizontal() || this.app.isSlim()) && !this.app.isMobile() && !this.app.isTablet()) {
